...
class Main { //Реализуем класс Main по ТЗ

    public static String[] calc(String inputString) { //реализуем методом public static String calc(String input) по ТЗ
        String[] calc_inputString = inputString.split(" "); //ввод строки в калькуляторе, должны разделяться пробелами
        if (calc_inputString.length != 3) { //вычисляем кол-во символов в сроке ввода, результат должен быть равен не менее 3
            Scanner Scanner = new Scanner(System.in); //сканирем строку ввода
            System.out.println("Проверьте наличие одного пробела между символами и наличие трех символов (одна цифра, выражение, одна цифра), пример: 2 + 5 или II + V"); //выдача в консоль инфо о неверном формате ввода
            inputString = Scanner.nextLine(); //выдает следующую строку для сканирования
            return calc(inputString);//возврат к запуску условия if
        } else { //еще
            return calc_inputString;//повтор ввода
        }
    }

    private static boolean its_an_arabic_numbers = true;

    public static void main(String[] args) { //создаем публичную статичную пустоту со стороковыми аргументами
        Scanner inputString_a_value = new Scanner(System.in); //создание строки для вода значений
        System.out.print("Input: "); //выводим в стороку печать текста
        String inputString = inputString_a_value.nextLine(); //в следующей строке выполняем проверки
        while (!inputString.isEmpty()) {
            ...
            Number values;
            int value1 = 0;
            int value2 = 0; //требование к целочисленным значениям
            try {
            ...
            System.out.println();
            System.out.print("Enter the next values: ");
